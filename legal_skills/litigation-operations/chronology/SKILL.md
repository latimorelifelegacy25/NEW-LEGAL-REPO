...
1. **User-provided export/path** — production folder, Bates manifest, eDiscovery export.
2. **eDiscovery connector** — if CLAUDE.md declares a platform and a connector is available, query by matter + custodian + date range.
3. **Ask** — same thin-source check.

For every source:
- Can I actually read it? (reachable, permission)
- Date range it covers
- Document types (email, contract, filing, internal memo, calendar)
- Approximate count

Build a source manifest before extracting. This goes in the output header.

### Step 1a: Disclosed-document use gate

If any source was obtained through disclosure or discovery, capture the user's confirmation that the proposed use is permitted under the applicable rule/order. If not confirmed, stop external-use variants (SoF / client / cross-matter use) and allow only an internal flagged working chronology if the user explicitly wants to proceed.

### Step 2: Authority/source attribution

The chronology is principally document-derived, but it can contain legal characterizations: limitations dates, discovery deadlines, privilege conclusions, "triggered duty" statements, etc. Those are not document facts. Tag them.

**No silent supplement.** If a legal research query through the configured legal research tool (Westlaw, CourtListener, Trellis, Descrybe, or firm platform) returns few or no results for an authority the chronology needs, report what was found and stop. Do not fill the gap from general web search or model memory without asking.

**Source attribution.** Tag each legal proposition with its source: `[Westlaw]`, `[CourtListener]`, `[Trellis]`, `[Descrybe]`, connector name, `[web search — verify]`, `[model knowledge — verify]`, or `[user provided]`.
